...
/* ModRM mode values */
typedef enum {
    MODRM_MODE_NO_DISP = 0,      /* Memory mode, no displacement */
    MODRM_MODE_DISP8 = 1,        /* Memory mode with 8-bit displacement */
    MODRM_MODE_DISP32 = 2,       /* Memory mode with 32-bit displacement */
    MODRM_MODE_REGISTER = 3      /* Register mode, no memory access */
} modrm_mode_t;
...
typedef struct {
    ...
    uint8_t valid_prefixes;      /* Valid prefix combinations (PRE_xxx) */
    uint8_t group_index;         /* Group index if applicable */
    imm_type_t imm_type;         /* Type of immediate value */
    uint8_t is_relative;         /* Whether uses relative addressing */
} opcode_info_t;
